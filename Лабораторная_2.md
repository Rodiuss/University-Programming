# Задание 1

> Дано действительное число a. Не пользуясь никакими другими операциями, кроме умножения, получить a2, a5, a17 за шесть операций.

```pascal
program FindOverflowN;

begin
  var a, aTemp: Real;
  a := ReadReal('Введите a:');
  aTemp := a;
  
  a *= a;
  writeln('a^2: ', a);
  
  a *= a;
  writeln('a^5: ', a * aTemp);
  
  a *= a;
  a *= a * aTemp;
  writeln('a^17: ', a);
end.
```

# Задание 2

> Найти площадь сектора, радиус которого равен 13,7, а дуга содержит заданное число радиан. Составьте программу, которая запрашивает ввод данных с клавиатуры. Ответ округлите до двух знаков после запятой.

```pascal
program SectorArea;

var
  r, phi, area: real;

begin
  writeln('Введите радиус круга (r): ');
  readln(r);
  
  writeln('Введите угол дуги в радианах (φ): ');
  readln(phi);
  
  area := 0.5 * r * r * phi;
  
  writeln('Площадь сектора: ', area:0:2);
end.

```

# Задание 3

> Даны действительные числа $x,y,z$. Вычислить max ($x+y+z$, $xyz$). Составить программу двумя способами: без использования функции max и  использованием функции max.

```pascal
program MaxWithoutFunction;

var
  x, y, z, sum, product, myResult: real;
  
begin
  writeln('Введите три действительных числа:');
  readln(x, y, z);

  sum := x + y + z;
  product := x * y * z;

  if sum > product then
    myResult := sum
  else
    myResult := product;

  writeln('Максимальное значение (without max): ', myResult:0:2);
  writeln('Максимальное значение (with max): ', Max(sum, product):0:2);
end.
```

# Задание 4

> Дано действительное число $a$. Для функций, графики которых представлены на рисунках, вычислить $f(a)$.
>
> ![lab_2_4.png](img/lab_2_4a.png)

```pascal
program Graph;

var a, y: real;

begin
  a := readReal('Введите число a: ');

  if a <= -1 then
    y := 1 / sqr(a)
  else if (a > -1.0) and (a <= 2) then
    y := sqr(a)
  else 
    y := 4;

  writeln('y = ', y);
end.
```

> Дано действительное число $a$. Для функций, графики которых представлены на рисунках, вычислить $f(a)$.
>
> ![lab_2_4.png](img/lab_2_4b.png)

```pascal
program Graph;

var x: Real;

procedure writeF(x: Real);
begin
  writeln('f(a) = ', x);
end;

begin
  x := readReal('Введите значени x:');
  
  if (x < -2) or (x > 2) then writeln('null')
  else if (x >= -2) and (x < -1) then writeF(-x - 1)
  else if (x >= -1) and (x < 0) then writeF(x + 1)
  else if (x >= 0) and (x < 1) then writeF(-x + 1)
  else writeF(x - 1);
end.
```

# Задание 5a

> Даны действительные числа $х, у$. Определить, принадлежит ли точка  c координатами $х, у$ заштрихованной части плоскости. 
>
> ![lab_2_5a.png](img/lab_2_5a.png)

```pascal
program PointInCircle;
var
  x, y: real;
  inRegion: boolean;
begin
  write('Введите координаты точки (x, y): ');
  readln(x, y);
  
  inRegion := (x*x + y*y) <= 4;
  
  if inRegion then
    writeln('Точка принадлежит заштрихованной области')
  else
    writeln('Точка НЕ принадлежит заштрихованной области');
end.
```

# Задание 5b

> Даны действительные числа $х, у$. Определить, принадлежит ли точка  c координатами $х, у$ заштрихованной части плоскости. 
>
> ![lab_2_5b.png](img/lab_2_5b.png)

```pascal
program PointInRectangle;
var
  x, y: real;
  inRegion: boolean;
begin
  write('Введите координаты точки (x, y): ');
  readln(x, y);
  
  inRegion := (x >= -2) and (x <= 2) and (y >= -1) and (y <= 1);
  
  if inRegion then
    writeln('Точка принадлежит заштрихованной области')
  else
    writeln('Точка НЕ принадлежит заштрихованной области');
end.
```

# Задание 6

> Дано натуральное число $n\ (n \le 100)$. Составьте программу, которая запрашивает число и определяет количество цифр в его записи. Если число не удовлетворяет условию, должно выводиться сообщение об ошибке ввода.

```pascal
program getDigitNumber;

var n: Integer;

begin
  try    
  n := readInteger('Введите натуральное число n:');
  except
    on System.FormatException do
      begin
        writeln('Ошибка ввода.');
        exit;
      end;
  end;
  
  if n > 100 then
    writeln('Ошибка ввода.')
  else if n = 100 then
    writeln('Число содержит 3 цифры')
  else if n >= 10 then
    writeln('Число содержит 2 цифры')
  else
    writeln('Число содержит 1 цифру')
end.
```

# Задание 7

> Поле шахматной доски определяется парой натуральных чисел, каждое из которых не превосходит восьми: первое число – номер вертикали (при счете слева направо), второе – номер горизонтали (при счете снизу вверх). Даны натуральные числа $k, l, m, n$, каждое из которых не превосходит восьми. Требуется выяснить, являются ли поля ($k,l$) и  ($m,n$) полями одного цвета?

```pascal
program Chests;

var 
  firstVertical,
  firstHorisontal,
  secondVertical,
  secondHorisontal: integer;
  
function getColor(isVertical: Boolean; isHorisontal: Boolean):Boolean;
begin
  if (isVertical and isHorisontal)
    or (not isVertical and not isHorisontal) then
    getColor := False // 'Темная'
  else if (isVertical and not isHorisontal)
    or (not isVertical and isHorisontal) then
    getColor := True; // 'Светлая'
end;

begin
  (firstVertical, firstHorisontal) :=
    ReadInteger2('(1) Введите номер вертикали, горизонтали (не больше 8):');
  (secondVertical, secondHorisontal) :=
    ReadInteger2('(2) Введите номер вертикали, горизонтали (не больше 8):');
  
  if (firstVertical > 8) or (firsthorisontal > 8)
    or (secondVertical > 8) or (secondhorisontal > 8)then
  begin
    writeln('Ошибка ввода');
    exit;
  end;
  
  writeln(getColor(firstVertical mod 2 = 0, firstHorisontal mod 2 = 0)
    = getColor(secondVertical mod 2 = 0, secondHorisontal mod 2 = 0))
      
end.
```
